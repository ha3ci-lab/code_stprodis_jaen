...
from speecht5.data.text_translation_dataset import TextTranslationDataset
...
def load_text_translation_dataset(
    data_path,
    split,
    src,
    src_dict,
...
TASK_NAME = ["s2t", "t2s", "s2s", "s2c", "pretrain", "text_translation"]
EVAL_BLEU_ORDER = 4
...
class SpeechT5Task(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=-1,
            help="the amount of upsample primary dataset"
        )
        parser.add_argument(
            "--left-pad-source",
            type=bool,
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help= "if >0, then bucket source and target lengths into N buckets and pad accordingly; this is useful on TPUs to minimize the number of compilations"
        )
        parser.add_argument(
            "--eval-bleu",
            action="store_true",
    ...
    def __init__(self, args, dicts, config):
        ...
        elif self.t5_task == "text_translation":
            self.max_pos = [self.args.max_source_positions, self.args.max_target_positions]
        if self.t5_task != "text_translation":
            ...
            self.dicts["src_dict"].add_symbol("<mask>")
            self.dicts["src_dict"].add_symbol("<ctc_blank>")
    ...
    def setup_task(cls, args, **kwargs):
        ...
        if args.t5_task == "pretrain":
        ...
        elif args.t5_task == "text_translation":
            ...
            dicts["src_dict"] = Dictionary.load(os.path.join(paths[0], "dict.{}.txt".format(args.source_lang)))
            dicts["tgt_dict"] = Dictionary.load(os.path.join(paths[0], "dict.{}.txt".format(args.target_lang)))

            assert dicts["src_dict"].pad() == dicts["tgt_dict"].pad()
            assert dicts["src_dict"].eos() == dicts["tgt_dict"].eos()
            assert dicts["src_dict"].unk() == dicts["tgt_dict"].unk()
            logger.info("[{}] dictionary: {} types".format(args.source_lang, len(dicts["src_dict"] )))
            logger.info("[{}] dictionary: {} types".format(args.target_lang, len(dicts["tgt_dict"])))
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if self.t5_task == "s2t":
            ...
            self.datasets[split] = load_text_translation_dataset(
                data_path,
                split,
                src,
                self.dicts["src_dict"],
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            ...
            if self.args.eval_bleu:
                ...
                agg_logging_output["_bleu_sys_len"] = bleu.sys_len
                agg_logging_output["_bleu_ref_len"] = bleu.ref_len
                # we split counts into separate entries so that they can be
                # summed efficiently across workers using fast-stat-sync
                assert len(bleu.counts) == EVAL_BLEU_ORDER
                for i in range(EVAL_BLEU_ORDER):
                    agg_logging_output["_bleu_counts_" + str(i)] = bleu.counts[i]
                    agg_logging_output["_bleu_totals_" + str(i)] = bleu.totals[i]
    ...
    def _inference_with_bleu(self, generator, sample, model):
        ...
        for i in range(len(gen_out)):
            ...
            refs.append(
                decode(
                    utils.strip_pad(sample["target"][i], self.dicts["tgt_dict"].pad()),
