...
from speecht5.criterions.text_translation_criterion import TextTranslationConfig, TextTranslationCriterion                                            
...
class SpeechT5Criterion(FairseqCriterion):
    def __init__(
        ...
        self.text_translation_criterion = TextTranslationCriterion(
            task,
            sentence_avg,
            label_smoothing,
            ignore_prefix_size,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        elif task_name == 'text_translation':
            return self.text_translation_criterion(model, sample, reduce)
    ...
    def reduce_metrics(cls, logging_outputs):
        ...
        for logging_output in logging_outputs:
            for task_name in logging_output:
                if task_name not in ['s2t', 't2s', 's2c', 's2s', 'text_pretrain', 'speech_pretrain', 'text_translation']:
                    continue
        ...
        for task_name in logging_outputs_dict:
            ...
            if task_name == "text_translation":
                text_translation_logging_outputs = logging_outputs_dict[task_name]
                loss_sum = sum(log.get("loss", 0) for log in text_translation_logging_outputs)
                nll_loss_sum = sum(log.get("nll_loss", 0) for log in text_translation_logging_outputs)
                ntokens = sum(log.get("ntokens", 0) for log in text_translation_logging_outputs)
                sample_size = sum(log.get("sample_size", 0) for log in text_translation_logging_outputs)
                ...
                total = utils.item(sum(log.get("total", 0) for log in text_translation_logging_outputs))
                if total > 0:
                    metrics.log_scalar("total", total)
                    n_correct = utils.item(
                        sum(log.get("n_correct", 0) for log in text_translation_logging_outputs)
                    )
