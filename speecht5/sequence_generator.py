...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        # initialize buffers
        ## CUDA ERROR with torch.tensor().to()
        # scores = (
        #     torch.zeros(bsz * beam_size, max_len + 1).to(src_tokens.device).float()
        # )
        ## changde to:
        scores = (
            torch.zeros(bsz * beam_size, max_len + 1).to(src_tokens.device).float()
        ...
        cands_to_ignore = (
            torch.zeros(bsz, beam_size).to(src_tokens.device).eq(-1)
...
class EnsembleModel(nn.Module):
    ...
    def is_t5_structure(self):
        ...
        t5_structure = hasattr(self.single_model, "text_encoder_prenet")
        return t5_structure
